...
def check_bias_labels(pred_array,actual_array):
    
    print("Total unique labels in prediction : ",len(np.unique(pred_array)))
    print("Total unique labels in actual : ",len(np.unique(actual_array)))
    
    assert len(np.unique(pred_array)) == len(np.unique(actual_array))

def check_all_class_pred(pred_array,actual_array):
    
    print("Labels in prediction : ",set(pred_array))
    print("Labels in actual : ",set(actual_array))

    assert len(set(pred_array)-set(actual_array))==0
